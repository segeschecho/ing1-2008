...
El pedido será encolado automáticamente de la forma que el sistema juzgue eficiente, pudiendo el encargado modificar dicho orden posteriormente. %TODO: como lo encolamos?
...
El sistema verifica si es posible satisfacer el pedido. En caso de no ser posible el sistema le envia un mensaje al cliente para avisar que el pedido no podra ser satisfecho, indicando que producto no esta disponible. Si el pedido sí podía ser tomado, el sistema lo registra como ingresado, genera un identificador de pedido (para despues hacer reclamos o consultas), estima el tiempo del mismo (desde que se registra el pedido hasta que llega a su casa, teniendo en cuenta el tiempo de demora del delivery), calcula el costo total y luego de todo esto procede a enviarle un mensaje de respuesta de sms con esos datos al cliente. Cada X tiempo, el sistema envía una actualización de status del pedido (esto puede ser personalizable al momento de registrar el usuario). %TODO: esto del x tiempo lo sacaria
...
\section{Cancelación}
La cancelación de un pedido se realiza via telefonica o personalmente. Una cancelación se puede realizar siempre y cuando no se haya entregado el pedido. %TODO: cual es el nombre de este estado

Los productos ya elaborados, se descartan al cancelarse el pedido. En cambio los insumos no utilizados regresan al stock.
Dado que no se informa cada producto que se prepara, por ejemplo si hay que hacer 6 empanadas el maestro no notifica cada vez que arma una, podria ocurrir que una
...
El primer elemento de la cola se considera lo proximo que el maestro debe preparar. El elemento que esta en esta posición no puede ser movido por el encargado de pedidos. El sistema muestra este pedido al maestro para que lo prepare, cuando este lo preparo completamente, se lo indica al sistema el cual muestra el proximo pedido en caso de que halla. Si no lo habia, en cuanto llegue uno se lo muestra al maestro.
...
El cambio de politica es ingresado por el encargado de pedidos, quien al cambiar la politica a sectores agiles, debe ingresar cuales son los modulos agiles. El sistema necesita ademas conocer el tamaño de los modulos de los hornos. Solo se puede cambiar la politica de cola cuando no hay pedidos en las colas de los hornos.
