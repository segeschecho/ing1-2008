...
\op{10. Si la tarjeta existe, el sistema pide al sistema bancario verificar el monto}{Sino el sistema muestra un mensaje de error al cliente. Ir al paso 4}
\op{11. El sistema registra la tarjeta}{}
\op{12. El sistema pide al sistema bancario verificar si la tarjeta tiene fondos}{}
\op{13. Si el pago con tarjeta es posible, el sistema espera que el cliente confirme el pedido}{Sino Ir al paso 2}
...
\op{16. Si el pedido es posible, ir al siguiente paso}{10. Si el pedido no es posible, se muestra mensaje de error al cliente indicando qu'e productos no estan disponibles. Ir al paso 2}
...
% Cancelando pedido
\op{1. El cliente informa al encargado de pedido su nombre o nombre de usuario y el id de pedido (ya sea via sms o tel'efono), o el cliente informa al mozo que desea cancelar un pedido y el mozo informa al encargado de pedidos el deseo de la mesa de cancelar el pedido}{}
\op{2. El encargado ingresa al sistema el nombre proporcionado por el cliente y el id de pedido}{}
\op{3. El sistema verifica que el pedido exista y que el cliente haya hecho el pedido}{}
\op{4. Si el pedido existe y est'a en estado preparando, se deben reingresar los insumos reutilizables. EXTIENDE caso de uso Agregando insumos reutilizables}{}
\op{4. Si el pedido existe y es de ese cliente, el pedido pasa a estado cancelado}{4. Si los datos no corresponden con ning'un pedido del cliente, el sistema muestra un cartel de error. Ir al paso 1}
\op{4. Fin CU}{}

\cu{Cancelando pedido}{Encargado de pedido}{$\infty$}{Pedido no entregado y cliente solicit'o una cancelacion de pedido}{Pedido cancelado}{Un cliente solicita cancelar un pedido, por tel'efono o personalmente, y el encargado de pedidos se encarga de esta cancelaci'on. Si el pedido se estaba preparando en el momento de la cancelaci'on, el maestro debe reingresar los insumos reutilizables} 
...
\op{3. El sistema muestra al cliente la lista de los pedidos del usuario pendientes de entrega cada uno con su estado correspondiente}{}
\op{4. Fin CU}{}
...
%TODO: FALTA EXPLICAR EN OPERATORIA SOBRE QUE PASA CON PEDIDOS DIFERIDOS, POR LO TANTO FALTA REVISAR ESTE CU
...
\op{6. Si el cliente est'a registrado, el sistema muestra al encargado el perfil del cliente}{6. Sino el encargado de pedidos le sugiere al cliente la posibilidad de registrarse. Si el cliente no lo desea entonces Ir a fin CU, sino se procede a registrarlo (EXTIENDE Registrando cliente)} %FIXME: este extiende parece ser razonable, pero no esta, revisar por favor
...
\op{8. Si el tel'efono registrado no coincide con el tel'efono desde el cual est'a llamando pero el cliente tiene usuario y clave, el encargado le pide al cliente que este indique usuario y clave y verifica}{7. Si el tel'efono registrado no coincide con el tel'efono desde el cual el cliente est'a llamando y el cliente no tiene usuario y clave ir a Fin CU}
...
\op{13. Si el cliente desea cambiar el pedido, ir al paso 9, sino el encargado confirma el pedido al sistema para que proceda a registrarlo}{}
...
%TODO: alto bache: tipo de despacho MOSTRADOR?????
...
\op{2. Si la factura es para pedido SMS o telef'onico, la forma de pago ser;a
\op{1. El sistema genera en el cajero el archivo con los datos necesarios para el sistema de facturación
\begin{itemize}
