...
La figura \ref{cicloPedido} nos ilustra los estados del pedido
...
Podemos considerar un caso simple donde un cliente ingresa un pedido y seguir al mismo hasta que termina. A continuación modelaremos mediante fsm el ciclo de vida de un pedido, sin entrar en grandes detalles, pero aun asi pudiendo ver cuestiones interesantes.

El escenario que vamos a modelar consiste en un cliente que hace pedidos de a uno, pudiendo cancelarlos, el encargado de pedido que realiza el ingreso de los mismos, y despacha los pedidos, el maestro que prepara y cocina el pedido, el sistema que monitorea el estado de los pedidos y el delivery.
...
Como dijimos esta version es simplificada, no tiene en cuenta la existencia de otros pedidos o clientes, otra formas de ingresar pedidos, composicion de los mismos, etc; sin embargo nos sirve para analizar ciertas cuestiones del ciclo de vida de un pedido.

En estas FSM podemos ver en que partes del ciclo de vida tienen relevancia los distintos agentes, asi es que podemos ver como el sistema es el que permite o no el ingreso de un pedido, rechazandolo en el caso de que el stock sea insuficiente, asi como tambien se encarga de avisar al maestro que prepare o cocine. Podemos ver como el sistema permite que el pedido avance hasta llegar a estar listo para despachar, como ademas monitorea casi en su totalidad las transiciones del pedido.

Otra cosa que debemos notar es como desde que ingresa hasta que es entregado el cliente puede todo el tiempo cancelar un pedido, y si bien en este diagrama se observa de una forma simplificada, la cancelación se verá mas adelante en la seccion Cancelación.
...
\begin{funcion}{RegistroUsuario}{nombreC: String, apellidoC: String, CalleC: String, NumeroC: Integer, LocalidadC: String, DepartamentoC: String, TelefonoC: Integer}{}{}
   \precond{Direccion.allInstances()->forall(d $|$ d.telefono \noig TelefonoC}
...
A continuacion presentamos un diagrama de actividad donde se pone de manifiesto las actividades que se llevan a cabo cuando el cliente desea cancelar un pedido.

\begin{figure}
\centering
\includegraphics[height=19cm]{./figuras/DiagramaActividadCancelacionPersonalYTelefonica.png}
\caption{Diagrama de actividad de las cancelaciones personales y telefonicas}
\end{figure}
...
Modelaremos la forma en la que se obtiene el proximo pedido a preparar luego de que el maestro informa que termino de preparar un pedido mediante operaciones, como necesitamos que devuelvan el proximo pedido, usaremos queries sin contexto. Supongamos que el que termino es el maestro pizzero y que hay algun pedido para entrar, la idea es buscar primero entre los mixtos que ya empezo el empanadero, y elegir de entre estos el que entro primero, ya que decidimos que una vez que entran a la cocina los pedidos se manejan de la forma First in First Out. Si no hay ninguno en preparacion se busca en la cola de ingreso. Si aca tampoco hay ninguno el sistema no avisa nada al maestro, pero para simplificar vamos a suponer que hay algun pedido en alguna de las dos colas.

\textbf{ProximoPedidoPizzero():Pedido} = if PedidosEnPreparacionEmpanadera()\flecha notEmpty() then PrimerPedidoPizzeroEnPreparacion() else PrimerPeddioPizzeroCola()
...
\textbf{PedidosPizzerosCola():Set(Pedido)} = Pedidos.allInstances() \flecha select( p $|$ p.estado = ingresado and p.productos.exists( pr $|$  pr.isKindOf(Pizza)))
...
Cuando uno de los maestros ingresa que termino de preparar se dispara la busqueda del proximo pedido a fin de notificarlo inmediatamente. Si no hay pedido, cuando llegue uno que pueda ser atendido por el maestro, este se le mostrara de inmediato.
...
El diagrama \ref{DiagramaProxPedido} nos permite ver las actividades que realiza el sistema cuando el maestro informa que termino un pedido. Notemos que la busqueda que realiza el sistema coincide con lo descripto por la operacion antes definida
...
\subsection{Estimacion de tiempos}
...
\begin{figure}[H]
\centering
\includegraphics[height=17cm]{./figuras/ContextoContingencia.png}
\end{figure}
\end{landscape}
...
%FIXME: Leer esto y revisar, esta en BETA
