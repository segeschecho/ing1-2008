...
La cocina es posiblemente el componente de mayor complejidad del sistema de la pizzería.
...
posterior entrega y facturación.
...
\begin{itemize}
\item Las clases de gestión y coordinación de cocina
\item Las clases relacionadas con la preparación de pedidos
\item Las clases relacionadas con los hornos
\end{itemize}

A continuación se describe la responsabilidad de cada uno de estos subcomponentes así
como las clases que lo componen y las tareas que las mismas tienen asignadas. Esta separación
en subcomponentes responde al mismo espíritu de la división en componentes del sistema global:
permite reducir el acoplamiento y aumentar la cohesión.

\subsection{Coordinación de cocina}

La coordinación de cocina cumple con el rol de interfaz hacia toda la cocina. En particular,
se encarga de redirigir los pedidos a la sección de preparación o de cocción según corresponda.
La decisión se toma en base a los atributos \verb0preparable0 y \verb0cocinable0 de los productos
contenidos en el pedido.

La interacción del coordinador de cocina con el resto del sistema es bidireccional. Esto
se hace para una  mayor flexibilidad en las políticas de cola de ingreso y cocina. Se permiten
dos operaciones:
\begin{itemize}
\item El Controlador de Ingreso intenta encolar un pedido en la cocina (se le devuelve un \verb0boolean0
      que indica si el pedido se encoló o no)
...
\end{itemize}
Esta doble interfaz permite que la cola de ingreso funcione con una política distinta a la
cola de cocina, permitiendo así, por ejemplo, que el encargado de pedidos reordene la cola
de ingreso según su criterio. Si bien para esto sería suficiente con que el coordinador
de cocina pida al coordinador de ingreso los pedidos, la llamada inversa permite que el
coordinador de cocina priorice pedidos según su tipo. Esto evita bloqueos innecesarios de
la preparación y mejora la eficiencia del sistema.

La doble interfaz produce una doble dependencia. Si bien se podría haber utilizado una
dependencia unidireccional con un \textit{callback} para la llamada inversa, optamos
por hacerlo de esta manera ya que resulta más claro y refleja el acoplamiento que hay
entre los dos coordinadores, que trabajan juntos estrechamente.

\subsection{Preparación de pedidos}

La preparación de pedidos consta de un Despachador de Preparación, una interface Preparador
implementada de momento por una única clase Preparador Especializado y una clase Aviso que
se utiliza según el patrón \textit{Observer} para enviar notificaciones desde los preparadores.

El Despachador de Preparación recibe del Coordinador  de Cocina los pedidos que debe
preparar. Esta abstracción responde a DIP y a \textit{Strategy} ya que permite reemplazar
la política de preparación para incluir una operatoria diferente.

El Despachador de Preparación Estándar, que implementa la política de preparación definida
por la especificación, consta de dos Preparadores idénticos, uno encargado de las pizzas y
otro de las empanadas. El despachador separa los pedidos según los tipos de los productos a
...
asincrónica. La clase Aviso engloba este \textit{callback}, del cual hay dos variantes:
...
permitiendo así reutilizar código y dar una excelente flexibilidad en cuanto a políticas
de preparación. Así se permitiría en un futuro, por ejemplo, incorporar preparadores que
fueran polivalentes (es decir, que no se especialicen en un único producto).
...
\subsection{Administración de hornos}

El subcomponente de horno se encarga de la operatoria de cocción de los pedidos,
incluída la política de las colas de horno. Consta de Despachador de Cocción, que
es el coordinador principal (e implementa mediante herencia las políticas de cola
de horno), y una serie de clases asociadas a cada horno: Horno, Controlador de Horno,
Fraccionador de Horno.

La política de colas de horno se implementó con herencia sobre el Despachador en
lugar de utilizar el patrón \textit{Strategy} ya que la política no debe ser
reemplazable ``al vuelo'', y la herencia nos proporciona una mayor facilidad de
acceso a los demás componentes del horno. El despachador se sirve de esta política
...
utilizado por el despachador, según SRP. Esta clase se encarga de registrar qué
módulos del horno están ocupados (y con qué productos), librando así de esta 
responsabilidad a la clase Horno, que solo debe almacenar información estática
sobre propiedades del horno.

Siguiendo la misma línea, el Fraccionador de Horno abstrae la tarea de segmentar
un pedido en grupos de productos que puedan cocinarse simultáneamente en un módulo.
El algoritmo utilizado para llevar a cabo esta tarea puede ser reemplazable, con
lo cual esta decisión de diseño está en consonancia con SRP y OCP.

Finalmente, cuando los pedidos han sido cocinados completamente el despachador de
cocción los vuelve a atribuir al coordinador de cocina, que por lo general los
...
\caption{Fin de preparación  se toma otro pedido de la cola interna}
\end{figure}
\end{landscape}
...
\caption{Fin de preparación y hay que pedir a la cola de ingreso}
\label{tePidoProx}
\end{figure}
\end{landscape}
...
\subsubsection{Fin de cocción de un pedido}
Cuando una parte termina de cocinarse, el despachador se pregunta si el pedido finalizó de cocinarse. En el caso en el que afirmativamente terminara de cocinarse, se debe pasar el pedido a la cola de listos. El diagrama de secuencias presente a continuación permite modelar este escenario, donde un pedido termina de cocinarse.
